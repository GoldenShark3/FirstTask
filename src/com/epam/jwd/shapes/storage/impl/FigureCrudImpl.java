...
public class FigureCrudImpl implements FigureCrud {
    ...
    @Override
    public void createFigure(String figureType, Point... points) throws FigureException {
        PolygonalFigure polygonalFigure = POLYGONAL_FIGURE_FACTORY.createFigure(figureType, points);
        POLYGONAL_FIGURE_STORAGE.fetchOrAddFigure(polygonalFigure);
    }

    @Override
    public void multiCreateFigure(List<? extends PolygonalFigure> listOfFigures) throws IllegalArgumentException {
        if (listOfFigures == null || listOfFigures.isEmpty()) {
            throw new IllegalArgumentException("List of figures must be not null");
        } else {
            POLYGONAL_FIGURE_STORAGE.addListOfFigure(listOfFigures);
        }
    }
